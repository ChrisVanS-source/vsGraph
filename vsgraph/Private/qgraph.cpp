...
template<class T>
void qgraph<T>::forward(qworkspace_view<T> ws) const
{
	get_impl(m_p)->forward(*get_impl(get_p(ws.get_ws())));
}

template<class T>
void qgraph<T>::backward(qworkspace_view<T> ws) const
{
	get_impl(m_p)->backward(*get_impl(get_p(ws.get_ws())));
}
